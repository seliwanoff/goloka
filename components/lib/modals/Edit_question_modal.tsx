...
import EditQuestionWidget from "../widgets/edit_question_widget";
...
const EditQuestionModal = ({ questions, id, action }: RearrangeProps) => {
  const { showEditQuestion, setShowQuestionEdit } = useEditAQuestion();

  return (
    <>
      {/* Edit CAMPAIGN  GROUP */}
      <Dialog open={showEditQuestion} onOpenChange={setShowQuestionEdit}>
        <DialogContent
        ...
        >
          ...
          <EditQuestionWidget question={questions} id={id} action={action} />
        </DialogContent>
      </Dialog>
    </>
  );
