...
import TaskSuccessModal from "../lib/modals/task_question_success";
import { questions4 } from "@/utils";
...
const QuestionFour: React.FC = () => {
  const { answers, updateAnswer, step, setStep, setAnswers } = useStepper();
  const [answer, setAnswer] = useState("");
  const [selectedValue, setSelectedValue] = useState("");
  const [open, setOpen] = useState<boolean>(false);

  const handleNext = () => {
    if (answer?.length > 1 && selectedValue?.length > 1 && step === 4) {
  ...
  useEffect(() => {
    setAnswer(answers.Q7);
    setSelectedValue(answers.Q8);
  }, []);

  return (
    <>
      <div className="space-y-5">
        {questions4.map((ques: any, index: number) => {
          return (
            <>
      ...
      <TaskSuccessModal open={open} setOpen={setOpen} />
    </>
  );
};
