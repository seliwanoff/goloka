import React, { useState } from "react";
import { TabsList, TabsTrigger, Tabs } from "../ui/tabs";
import { supportTabs } from "@/utils";
import { ChevronRight } from "lucide-react";
import { ArrowLeft } from "iconsax-react";
import ContactTab from "./contact_tab";
import HelpTab from "./help_tab";
import ChatTab from "./chat_tab";
import ReportTab from "./report_tab";
  
const supportTabContent = [
{
    title: "Contact Goloka",
    label: <ContactTab />,
    value: "contact",
  },
  {
    title: "Help center",
    label: <HelpTab />,
    value: "help",
  },
  {
    title: "Chat with Goloka",
    label: <ChatTab />,
    value: "chat",
  },
  {
    title: "Report an issue",
    label: <ReportTab />,
    value: "report",
  },
];

const MobileSupport = () => {
  const [tab, setTab] = useState<number>(-1);

  const supportStepper = () => {
    switch (tab) {
    }
  };

  return (
    <div className="absolute left-0 top-0 h-svh w-full bg-white px-4 pt-24 md:hidden">
      {tab >= 0 ? (
        <div className="no-scrollbar overflow-y-auto">
          <div className="">
            <div
              onClick={() => setTab(-1)}
              className="group inline-flex cursor-pointer items-center gap-2 pb-3.5 md:hidden"
            >
              <span>
                <ArrowLeft
                  size={24}
                  className="text-[#333333] group-hover:text-main-100"
                />
              </span>
              <p className="text-lg font-medium text-[#333333] group-hover:text-main-100">
                {supportTabContent[tab]?.title}
              </p>
            </div>

            <div className="mt-4">
              {/* {tab} {typeof tab} */}
              {supportTabContent[tab]?.label}
            </div>
          </div>
        </div>
      ) : (
        <Tabs
          defaultValue={"-1"}
          className="grid w-full gap-5 p-0 md:grid-cols-[1fr_2fr]"
        >
          <TabsList className="block h-auto space-y-4 bg-transparent p-0">
            {supportTabs?.map((tab: any, index: number) => (
              <TabsTrigger
                value={index.toString()}
                key={index}
                onClick={() => setTab(index)}
                className="flex h-[50px] w-full items-center justify-between rounded-full border border-[#E0E0E0] bg-[#F8F8F8] text-base font-medium text-[#071E3B] data-[state=active]:border-main-100 data-[state=active]:bg-[#3365E305] data-[state=active]:text-main-100"
              >
                {tab.label}

                <span>
                  <ChevronRight color="#808080" size={20} />
                </span>
              </TabsTrigger>
            ))}
          </TabsList>
        </Tabs>
      )}
    </div>
  );
};

export default MobileSupport;
