...
import { getAllTask, getBookmarkedCampaign } from "@/services/contributor";
...
const BookmarkPage: React.FC<ComponentProps> = ({}) => {
  ...
  const {
    ...
    queryKey: ["Get bookmark  list", currentPage, searchParams.toString()],
    queryFn: () =>
      getBookmarkedCampaign({
        page: currentPage,
        per_page: 9,
  ...
  return (
    <>
      <section className="pb-10 pt-[34px]">
        <h1 className="mb-6 text-2xl font-semibold text-[#333]">Bookmarks</h1>
          ...
          <div className="mx-auto mt-9 flex max-w-96 flex-col items-center lg:mt-[100px]">
            ...
            <p className="text-center text-base text-[#4F4F4F]">
              {searchParams.toString()
                ...
                : "There are no bookmarked campaigns presently, bookmark some to see them"}
            </p>
            <button
